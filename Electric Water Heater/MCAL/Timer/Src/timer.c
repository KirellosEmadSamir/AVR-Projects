...
{	
	...
	uint32 counter_us;
	counter_us=(timer_cgf->CPU_clk /MICRO_SECOND_INVERSE)*timer_cgf->resolution_us;
	...
	switch(timer_cgf->ch)
	{
		case CH0:
		{
			...
			case COUNTER_FALLING:
				...
				GpioPinDirection(TIMER0_EXT_CLK_PIN_PORT,TIMER0_EXT_CLK_PIN,INPUT);
...
void timer_SetCallBack(Timer_ch_E ch, pFun pf )
{
	...
	{
		case CH0:
			Timer0_ISR[TIMER0_COMP_ISR_INDEX] = pf;
			break;
		default:
		break;
	}
...
INT_HANDLER(TIMER0_COMP_VECT)
{
	...
	//Timer0_Comp_Hock_ISR();
	Timer0_ISR[TIMER0_COMP_ISR_INDEX]();
